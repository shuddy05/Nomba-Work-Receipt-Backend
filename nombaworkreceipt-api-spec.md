...
| Web framework | Fiber |
| Database | MySQL |
| Database access | Prisma Client Go (`github.com/steebchen/prisma-client-go`) — schema-first ORM, generated type-safe client |
...
> Do NOT introduce Redis, PostgreSQL, Supabase, MongoDB, or any tool not listed above without explicit confirmation. Note: `prisma-client-go` may pull in a MongoDB driver as an indirect dependency (it supports multiple databases internally) — this is expected and does NOT mean MongoDB is in use. The project uses MySQL exclusively. Do not write any code that imports or calls the Mongo driver directly.

> **ORM risk note:** `prisma-client-go` is a community-maintained package, not officially part of Prisma. It is less actively maintained than mainstream Prisma (TypeScript). This is a deliberate, accepted risk for this hackathon. Section 2.1 below defines a mandatory verification step before any real code is built on top of it. If it proves unstable, the fallback is `database/sql` with the standard MySQL driver — the full schema in Section 4 is plain SQL and works identically either way.

---

## 2.1 MANDATORY FIRST CHECK — Verify Prisma Client Go Before Building Anything
...
2. Write a `schema.prisma` file that matches the 8 tables in Section 4 exactly — same fields, same types, same relations. Do not let Prisma generate its own schema design; it must match Section 4 field-for-field.
3. Run `go run github.com/steebchen/prisma-client-go generate` — confirm it generates a Go client without errors.
4. Run `go run github.com/steebchen/prisma-client-go db push` against the actual Hostinger MySQL connection — confirm the tables are created successfully and match Section 4.
...
6. Remove the stray `go.mongodb.org/mongo-driver` entry from `go.mod` if it is not actually required — run `go mod tidy` after Prisma generation to clean up unused indirect dependencies.

**If any step fails:** stop, report the exact error, and do not attempt workarounds or partial fixes that "mostly" work. Bring the failure back for a decision — at that point the fallback is `database/sql` with the standard `go-sql-driver/mysql` package, using Section 4's schema directly via raw SQL migrations as originally written.
...
├── prisma/
│   └── schema.prisma                 — Prisma schema, must match Section 4 exactly
├── cmd/
│   └── api/
...
│   ├── database/
│   │   └── prisma.go                 — Prisma client initialization, connection lifecycle
│   ├── middleware/
│   │   ├── auth.go                   — JWT verification, role checks
│   │   ├── logger.go                 — structured request logging
│   │   └── cors.go                   — CORS config for Flutter app
│   ├── models/                       — NOTE: Prisma auto-generates its own typed models
│   │   └── (do not hand-write structs that duplicate Prisma's generated types — import
│   │       and use the generated client types directly in handlers/services instead.
│   │       Use this folder only for request/response DTOs that are NOT 1:1 with a table,
│   │       e.g. CreateJobRequest, JobResponse with nested escrow/merchant/buyer info.)
│   ├── handlers/
│   │   ├── auth_handler.go
│   │   ├── job_handler.go
│   │   ├── escrow_handler.go
...
├── migrations/
│   └── schema.sql                    — reference copy of the full schema (Section 4), kept in
│                                        sync manually. Prisma manages actual migrations via
│                                        `prisma db push` / `prisma migrate` against schema.prisma —
│                                        this file is documentation, not what's executed.
├── .env.example
├── .gitignore
├── go.mod
├── go.sum
...
## 4. Database Schema — Authoritative Reference (Express as Prisma Schema)

> This is the single source of truth for every table, field, type, and relationship in the
> database. Originally written as plain MySQL migrations — now expressed as `prisma/schema.prisma`
